...
### 3. Register the language (2 changes)

**a) Register in `src/js/localeMixin.js`:**

Add your language to the `availableLocales` array:
```js
availableLocales: [
...
**b) Register in `render.sh`:**

The build script generates a locale JS file from `src/locales/en.json`. When building for multiple languages, you'll loop over them:

```bash
for lang in en es; do
  echo "window.__locale = $(cat src/locales/$lang.json);" > "$LOCALE_JS_FILE"
  # ... rest of the build with npx pug3 -O "{ lang: '$lang' }"
done
```
...
- **Do not edit the Pug templates**: All translatable content is in the JSON locale files. The Pug templates only contain structural markup and `$t('key')` calls.
