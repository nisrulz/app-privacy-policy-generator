...
def download_and_cache_image(url, local_path):
    if not os.path.exists(local_path) or datetime.fromtimestamp(os.path.getmtime(local_path)) < ONE_WEEK_AGO:
...
def process_comment_images(comments):
    image_pattern = re.compile(r'(https?://(?:[^\s/$.?#].[^\s]*)\.(?:png|jpg|jpeg|gif))')
    for comment in comments:
        comment["created_at_formatted"] = datetime.strptime(comment["created_at"], "%Y-%m-%dT%H:%M:%SZ").strftime("%d %b %Y")
